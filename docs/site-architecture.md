...
/#stack           Stack — categorized, terminal-styled (Backend/JVM, Go, ML/Python, Infra)
...
/resume           Resume — inline HTML content + downloadable PDF (existing LaTeX resume)
/#contact         Contact — plain mailto fallback + LinkedIn/Telegram, no phone in cleartext
```

Anchors (`/#section`) keep it a single scrollable page for the primary sections (per requirements doc §2), matching the "pages" decision above while avoiding a full multi-route site; `/resume` is the one real sub-route, since it needs its own shareable URL and print-friendly layout distinct from the scroll page.

## 3. Navigation

Fixed/sticky top bar, visible at all scroll positions, with explicit labeled links (not icons-only): `Главная` · `Стек` · `Проекты` · `Опыт` · `Резюме` · `Контакты`. Active-section highlighting on scroll. No hamburger collapse above mobile breakpoints — the whole point of the therobin.dev-style reference is that the nav itself communicates the site's structure at a glance.
...
│   ├── sections/               # Hero, Stack, Projects, Experience, Contact
...
2. Build: `next build` (static export) on the VPS or via a simple CI step.
3. Serve the exported `out/` directory via Nginx, HTTPS via Certbot, matching the anti-pattern fix in requirements doc §9 (anpilovml.ru refuses HTTPS — don't repeat that).
4. `resume.pdf` is regenerated from `resume/*.tex` and copied into `public/` as part of the build, not hand-uploaded, so the downloadable PDF never drifts from the LaTeX source of truth.
